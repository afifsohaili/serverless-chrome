...
class ServerlessChrome {
      ...
      if (packagedIdividually) {
        ...
        yield Promise.all(functionsToCopyTo.map((() => {
          var _ref = _asyncToGenerator(function* (functionName) {
            yield fs.copy(path.join(servicePath, 'node_modules/@serverless-chrome/lambda/dist/headless-chromium'), path.resolve(servicePath, `.webpack/${functionName}/headless-chromium`));
          });
        ...
        yield fs.copy(path.join(servicePath, 'node_modules/@serverless-chrome/lambda/dist/headless-chromium'), path.resolve(servicePath, '.webpack/service/headless-chromium'));
      }
    })();
  }
  ...
  beforeCreateDeploymentArtifacts() {
    ...
    return _asyncToGenerator(function* () {
      ...
      if (_this2.messWithServicePath) {
        config.servicePath = path.join(_this2.originalServicePath, BUILD_FOLDER);
        ...
        if (!fs.existsSync(path.resolve(path.join(BUILD_FOLDER, 'node_modules')))) {
          fs.symlinkSync(path.resolve('node_modules'), path.resolve(path.join(BUILD_FOLDER, 'node_modules')), 'junction');
        }
        ...
        files.forEach(function (filename) {
          const sourceFile = path.resolve(path.join(_this2.originalServicePath, filename));
      ...
      yield Promise.all(functionsToWrap.map((() => {
        var _ref2 = _asyncToGenerator(function* (functionName) {
          ...
          });const wrapperTemplate = yield utils.readFile(path.resolve(__dirname, '..', 'src', wrapperTemplateMap[`${providerName}-${runtime}`]));
          ...
          yield fs.move(path.resolve(handlerCodePath, fileName), path.resolve(handlerCodePath, originalFileRenamed));

          // Write the wrapper code to the function's handler path
          yield utils.writeFile(path.resolve(handlerCodePath, fileName), wrapperCode);
        });
  ...
  afterCreateDeploymentArtifacts() {
    ...
    return _asyncToGenerator(function* () {
      if (_this3.messWithServicePath) {
        ...
        yield fs.copy(path.join(_this3.originalServicePath, BUILD_FOLDER, SERVERLESS_FOLDER), path.join(_this3.originalServicePath, SERVERLESS_FOLDER));
  ...
  cleanup() {
    ...
    return _asyncToGenerator(function* () {
      ...
      fs.removeSync(path.join(_this4.originalServicePath, BUILD_FOLDER));
    })();
  }
}

module.exports = ServerlessChrome;
